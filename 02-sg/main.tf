module "mongodb" {
    source =  "git::https://github.com/KodukG01/terraform-aws-securitygroup.git?ref=main"
    project = var.project
...
resource "aws_security_group_rule" "mongodb_ports_bastion" {
  count = length(var.mongodb_ports_vpn)
  type = "ingress"
  from_port = var.mongodb_ports_vpn[count.index]
  to_port = var.mongodb_ports_vpn[count.index]
...
resource "aws_security_group_rule" "redis_ports_bastion" {
    count = length(var.redis_ports_vpn)
    type = "ingress"
    from_port = var.redis_ports_vpn[count.index]
    to_port =  var.redis_ports_vpn[count.index]
...
resource "aws_security_group_rule" "mysql_ports_bastion" {
    count = length(var.mysql_ports_vpn)
    type = "ingress"
    from_port = var.mysql_ports_vpn[count.index]
    to_port = var.mysql_ports_vpn[count.index]
...
resource "aws_security_group_rule" "rabbitmq_ports_bastion" {
    count = length(var.rabbitmq_ports_vpn)
    type = "ingress"
    from_port = var.rabbitmq_ports_vpn[count.index]
    to_port = var.rabbitmq_ports_vpn[count.index]
...
resource "aws_security_group_rule" "catalogue_backend_bastion" {
    type = "ingress"
    from_port = 22
    to_port = 22
    protocol = "tcp"
